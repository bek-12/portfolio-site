...
const focusStyle = { borderColor: '#C9A84C', boxShadow: '0 0 0 1px rgba(201,168,76,0.2)' };

function inputStyle(hasError) {
  return {
    background: '#0a0a0a',
    border: `1px solid ${hasError ? 'rgba(239,68,68,0.6)' : 'rgba(201,168,76,0.2)'}`,
  };
}

// ─── InputField — defined at MODULE LEVEL so React never recreates it ─────────
// Receives form values, errors, and onChange as props instead of closing over them.
function InputField({ field, type = 'text', placeholder, label, required, value, error, onChange }) {
  return (
    <div>
      ...
      <input
        ...
        onBlur={(e) => Object.assign(e.target.style, inputStyle(error))}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="p-8 rounded-2xl" style={{ background: '#111111', border: '1px solid rgba(201,168,76,0.15)' }}>
                ...
                {CONTACT_INFO.map(({ icon: Icon, label, value, href }) => (
                  <div key={label} className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      {href ? (
                        <a href={href} className="text-sm text-[#A0A0A0] transition-colors"
            ...
            <div className="p-6 rounded-2xl"
              style={{ background: 'linear-gradient(135deg, rgba(201,168,76,0.1), rgba(201,168,76,0.04))', border: '1px solid rgba(201,168,76,0.2)' }}>
          ...
          <div className="lg:col-span-3">
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} noValidate className="p-8 rounded-2xl space-y-5"
                style={{ background: '#111111', border: '1px solid rgba(201,168,76,0.15)' }}>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <InputField
                    field="fullName" label="Full Name" placeholder="Bereket Mebratu" required
                    value={form.fullName} error={errors.fullName} onChange={handleChange}
                  />
                  <InputField
                    field="companyName" label="Company Name" placeholder="Acme Pharmacy" required
                    value={form.companyName} error={errors.companyName} onChange={handleChange}
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <InputField
                    field="email" type="email" label="Email Address" placeholder="you@company.com" required
                    value={form.email} error={errors.email} onChange={handleChange}
                  />
                  <InputField
                    field="phone" type="tel" label="Phone Number" placeholder="+251 9XX XXX XXX"
                ...
                {/* Interested In */}
                <div>
                  <label className="block text-sm font-medium text-[#A0A0A0] mb-1.5">Interested In</label>
                  <select
                    name="interestedIn"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    style={{ background: '#0a0a0a', border: `1px solid ${errors.message ? 'rgba(239,68,68,0.6)' : 'rgba(201,168,76,0.2)'}` }}
                    onFocus={(e) => { e.target.style.borderColor = '#C9A84C'; e.target.style.boxShadow = '0 0 0 1px rgba(201,168,76,0.2)'; }}
                    onBlur={(e) => { e.target.style.borderColor = errors.message ? 'rgba(239,68,68,0.6)' : 'rgba(201,168,76,0.2)'; e.target.style.boxShadow = 'none'; }}
                  />
