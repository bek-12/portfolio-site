      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full" style={statusStyles[request.status] || statusStyles.new}>
              {request.status}
            </span>
          </div>
          <div className="flex flex-wrap items-center gap-3 mt-0.5 text-xs text-[#555]">
            <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{request.companyName}</span>
            <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{date}</span>
            {request.interestedIn && <span style={{ color: '#C9A84C' }}>→ {request.interestedIn}</span>}
          </div>
        </div>
        ...
        <button onClick={() => setExpanded(!expanded)} className="p-1.5 text-[#555] hover:text-white transition-colors">
      ...
      {expanded && (
        <div className="px-5 pb-5 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4"
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <a href={`mailto:${request.email}`} className="text-[#A0A0A0] truncate transition-colors"
              onMouseEnter={(e) => (e.target.style.color = '#C9A84C')}
              onMouseLeave={(e) => (e.target.style.color = '#A0A0A0')}>{request.email}</a>
          </div>
          {request.phone && (
            <div className="flex items-center gap-2 text-sm">
              <Phone className="w-4 h-4 shrink-0" style={{ color: '#C9A84C' }} />
              <a href={`tel:${request.phone}`} className="text-[#A0A0A0] transition-colors"
                onMouseEnter={(e) => (e.target.style.color = '#C9A84C')}
                onMouseLeave={(e) => (e.target.style.color = '#A0A0A0')}>{request.phone}</a>
            </div>
          )}
          {request.message && (
            <div className="sm:col-span-2">
              <div className="text-xs text-[#555] mb-1">Message</div>
              <p className="text-sm text-[#A0A0A0] rounded-xl p-4 leading-relaxed"
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-[#555]">
            {search || filterStatus !== 'all' ? 'No requests match your filters.' : 'No demo requests yet.'}
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map((r) => <RequestRow key={r.id} request={r} onStatusChange={handleStatusChange} />)}
          </div>
        )}
      </div>
    </AdminLayout>
